 ...
 * 
 */
public class DbxImpSelectPageMethodGenerator extends AbstractJavaServiceGenerator {

	public final static String THIS_METHOD_NAME = "dbxSelectByExample";

	public DbxImpSelectPageMethodGenerator() {
		super(true);
	}
	...
	public void addMethod(TopLevelClass topLevelClass) {
		...
		method.addBodyLine(MessageFormat.format("SoaResult<{0}> res=new SoaResult<{0}>();", resParameterType.getShortName()));
		method.addBodyLine("try {");
		method.addBodyLine(MessageFormat.format("{0} e = new {1}();", examp.getShortName(), examp.getShortName()));
		method.addBodyLine(MessageFormat.format("List<{0}> list={1}.selectByExample(e);", introspectedTable.getRules()
				.calculateAllFieldsClass().getShortName(), JavaBeansUtil.firstLetterLower(fieldType.getShortName())));

		method.addBodyLine("if (list.size() == 1){");
		method.addBodyLine("res.setCode(0);");
		method.addBodyLine("AppCustomerRes appCustomerRes=new AppCustomerRes();");
		method.addBodyLine("SpringBeanUtils.copyProperties(list.get(0), appCustomerRes);");
		method.addBodyLine("}else{");
		method.addBodyLine("res.setCode(-2);");
		method.addBodyLine("res.setMsg(\"not found or target exies mutl\");");
		method.addBodyLine("}");
