...
public final class PluginAggregator implements Plugin {
    ...
    public List<GeneratedUIFile> contextGenerateAdditionalUIFiles(
            IntrospectedTable introspectedTable) {
        List<GeneratedUIFile> answer = new ArrayList<GeneratedUIFile>();
        for (Plugin plugin : plugins) {
            List<GeneratedUIFile> temp = plugin
                    .contextGenerateAdditionalUIFiles(introspectedTable);
            if (temp != null) {
                answer.addAll(temp);
            }
        }
   ...
   // @Override
    public boolean uiGenerated(GeneratedUIFile gxf, IntrospectedTable introspectedTable)
    {
        ...
        for (Plugin plugin : plugins) {
            if (!plugin.uiGenerated(gxf, introspectedTable)) {
                rc = false;
                break;
            }
        }
